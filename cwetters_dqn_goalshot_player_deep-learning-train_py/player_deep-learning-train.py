...
##############################################################################
            ...
            for i in range(5):
                ...
                position = [i, round(cx/2.05, 2), round(cy/1.35, 2),
                            ...
                            round(cby/1.35, 2), 0, 0]
                # Example: using the normalized coordinates for robots and ball
                if(mag != 0):
                    position = [i, round(cx/2.05, 2), round(cy/1.35, 2),
                            round(ct/(2*math.pi), 2), round(cbx/2.05, 2),
                            round(cby/1.35, 2), round(delx/mag,2), round(dely/mag,2)]
...
##############################################################################
            ...
            if len(self.D[0]) >= self.observation_steps:
                ...
                batch_phy = np.zeros((self.minibatch_size, 8)) # depends on what is your input state
                batch_phy_ = np.zeros((self.minibatch_size, 8)) # depends on what is your input state
                for i in range(self.minibatch_size):
                    ...
                    batch_phy[i] = self.D[player][index][0].reshape((1,8)) # depends on what is your input state
                    batch_phy_[i] = self.D[player][index+1][0].reshape((1,8)) # depends on what is your input state
