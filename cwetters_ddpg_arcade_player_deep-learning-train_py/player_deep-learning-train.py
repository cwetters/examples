...
##############################################################################
    ...
    def on_event(self, f):
        ...
        def set_action(a):
            #self.printConsole(a)
            for i in range (int(self.action_dim/2)):                
                maxspeed = self.max_linear_velocity[i]
                #bound to -1,1
                vel = max(min(1.0, a[0][2*i]), -1.0)
                side = max(min(1.0, a[0][2*i + 1]), -1.0)

                self.wheels[2*i] = vel*min(1.0, 2*side + 1)
                self.wheels[2*i +1] = -vel*min(1.0, 1 - 2*side)
...
##############################################################################
            ...
            if not (self.resetting or self.timedout):
                ...
                action = self.actor.predict(np.reshape(position, (1, self.actor.s_dim))) # + noise
            else:
            ...
            if not (self.resetting or self.timedout and False):
