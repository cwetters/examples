...
private:
  ...
  void goalPosUpdate() //update goal position to be in a scoop formation around current ball location pointed goal-wards
  {
    ...
    for (int i = 0; i < 5; i++) // set relative positions rotated around point p by direction theta
    {
      ...
      if (i == 0){ // goalie
        scale = 0.0;
        //scale = std::min((cur_ball[0]+3)/3.0, 1.0);
        //scale = std::max((cur_ball[0]+3)/3, -0.05);
      } else if (i < 3){ // defenders
        //scale = std::min((cur_ball[0] +3)/8.0 + 0.5, 1.0);
        //scale = std::max((cur_ball[0] +3)/8.0 + 0.5, 0.5);

      } else { // forwards
        //scale = std::min(-1.0*cur_ball[0] + 1.0, 1.0);
      }

      double ctheta = std::cos(goal_dir[i]);
      double stheta = std::sin(goal_dir[i]);
      goal_pos[i][0] = cur_ball[0] + scale*goal_rel[i][0]*ctheta - scale*goal_rel[i][1]*stheta;
      goal_pos[i][1] = cur_ball[1] + scale*goal_rel[i][1]*ctheta + scale*goal_rel[i][0]*stheta;
      goal_pos[i][2] = goal_dir[i];
      if (i == 0) // goalie
      {
        goal_pos[i][0] = std::min(2.4, goal_pos[i][0]);
        goal_pos[i][0] = std::max(-3.8, goal_pos[i][0]);
        
      } else if (i < 3) { // defenders
        goal_pos[i][0] = std::min(2.7, goal_pos[i][0]);
        goal_pos[i][0] = std::max(-3.5, goal_pos[i][0]);
      } else { // forwards
        goal_pos[i][0] = std::max(-2.5, goal_pos[i][0]);
      }
